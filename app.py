...
if submit_button:   #request.get
    ...
    prediction = requests.post(url = "http://127.0.0.1:8000/predict", data = json.dumps(input_fields))
    #print(prediction.json())
    prediction = prediction.json()['Probability of Booking']

    # Display prediction in fancy way using Plotly
    fig = px.pie(values=[prediction, 100-prediction], names=['Probability of Booking', 'Probability of Not Booking'])
    st.plotly_chart(fig)
